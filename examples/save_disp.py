...
def parse_args(arg_list: List[str] = sys.argv[1:]) -> Namespace:
	...
	# Optional arguments.
	parser.add_argument("--device",			type=int,	default=-1,	help="Device to run the model on. Use -1 for CPU, or a non-negative integer for CUDA device index.")
...
def main() -> None:
	...
	if args.device == -1:
		...
		assert torch.cuda.is_available(), "CUDA is not available, but a CUDA device was requested"

		num_cuda_devices = torch.cuda.device_count()
		assert args.device < num_cuda_devices, f"CUDA device index {args.device} is out of range. Available device max index: {num_cuda_devices - 1}"
